      ...
      <div class="min-w-0 flex-1">
        ...
        <dl class="mt-2 grid min-w-0 gap-x-6 gap-y-1.5 text-xs sm:grid-cols-2">
          <div
            ...
            class="min-w-0"
          >
            <dt class="text-base-content/45">{{ $t('scriptURL') }}</dt>
            <dd
          ...
          <div class="min-w-0">
            <dt class="text-base-content/45">{{ $t('scriptPath') }}</dt>
            <dd
              class="mt-0.5 font-mono text-[11px] break-all"
              :title="item.path"
            >
          ...
          <div v-if="item.type === 'http'">
            <dt class="text-base-content/45">{{ $t('scriptUpdateInterval') }}</dt>
      ...
      <label class="flex shrink-0 cursor-pointer items-center gap-2 pt-0.5">
        ...
        <span class="relative flex h-5 items-center">
          <input
            ...
            :aria-label="$t(targetEnabled ? 'disableScript' : 'enableScript', { name: item.name })"
            :disabled="isUpdating"
            @change="toggleScript"
          />
          <span
...
import { setScriptEnabled } from '@/assembly/scripts'
...
const updatedAt = computed(() =>
  props.item.updatedAt ? fromNow(props.item.updatedAt) : t('scriptNeverUpdated'),
)
...
const toggleScript = async () => {
  ...
  try {
    await setScriptEnabled(props.item.name, enable)
  } catch {
